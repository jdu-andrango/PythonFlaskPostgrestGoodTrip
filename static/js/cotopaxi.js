...
window.addEventListener('DOMContentLoaded', async () => {
    const response = await fetch("/goodtrip/comentarioCotopaxi")
...
form.addEventListener('submit', async e => {
    ...
    if (!editar) {
        ...
        const response = await fetch(`/goodtrip/comentarioCotopaxi/${comentarioId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
function renderComentario(comentarios) {
    ...
    comentarios.forEach(comentario => {
        ...
        btnDelete.addEventListener('click', async () => {
            ...
            const response = await fetch(`/goodtrip/comentarioCotopaxi/${comentario.id}`, {
                method: 'DELETE',
            })
            const data = await response.json()
            console.log(data)
